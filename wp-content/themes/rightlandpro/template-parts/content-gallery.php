    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('postbox post format-image mb-40'); ?> >
        ...
        if (!empty($images)) : ?>
            ...
            <?php foreach( $images as $image ) :  ?>
                <img src="<?php echo esc_url($image); ?>" alt="<?php print esc_attr__('image','rightlandpro'); ?>" />
            <?php endforeach; ?>
            </div>
        <?php endif; ?>
...
<?php
    ...
    <article id="post-<?php the_ID(); ?>" <?php post_class('postbox post format-gallery mb-40'); ?> >
        
        <?php $images =  get_post_meta(get_the_id(), '_gallery-images', true); 
        if (!empty($images)) : ?>
            <div class="postbox__gallery mb-30">
            <?php foreach( $images as $image ) :  ?>
                <img src="<?php echo  esc_url($image); ?>" alt="<?php print esc_attr__('image','rightlandpro'); ?>" />
            <?php endforeach; ?>
            </div>
